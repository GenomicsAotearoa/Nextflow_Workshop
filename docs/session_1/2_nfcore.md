...
!!! question "Exercise"
    ...
    ??? success "Solution"
        ...
        !!! warning
            ...
            Some commands may change depending on the operating system you are using.
...
A multi-step pipeline is required to demonstrate cache and resume. The [`nf-core/demo`](https://github.com/nf-core/demo) pipeline was created with the nf-core `create` command and has the same structure as nf-core pipelines. It is a toy example with 3 processes:
...
!!! question "Exercise"
    ...
    Use Nextflow to `pull` the `nf-core/demo` workflow directly from GitHub:

    ??? success "Solution"

        Use Nextflow to `pull` the `demo` workflow from the `nf-core` GitHub repository:

        ```bash
        nextflow pull nf-core/demo -r 1.1.0
        ```
